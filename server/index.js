...
const main = async () => {
  ...
  const apply_action_scan = async (data) => {
    const calling_client_response = data['client'] || client.broadcast_message;
    ...
    let work_order = await database.get_work_order(woid).catch(async (error) => {
      ...
      await calling_client_response("server_error", {error: error, message: "No message available."});
      return;
    });
    if (!work_order) {
      logger.error(`Failed to get work order ${woid} - aborting action request.`);
      await calling_client_response("server_error", {
  ...
  client.emitter.on("apply_action", async (data) => {
    await apply_action_scan(data);
  })

  scanner.emitter.on("qr_apply_action", async (data) => {
    await apply_action_scan({
      "data": data
    });
  })
